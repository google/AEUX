...
var AEUX = (function () {
    ...
    function aeGroup(layer, opt_parent) {
        ...
        if (prefs.precompGroups || layer.type == 'Component') {
            ...
            var groupComp = app.project.items.addComp(nameInc(layer.name, app.project.items), Math.max(Math.round(layer.frame.width * compMult), 4), // x size
    ...
    function aeCompound(layer, opt_parent) {
        ...
        } // add a merge paths if one isn't already there
        addStroke(r, layer);
        addFill(r, layer);
        addDropShadow(r, layer);
        addBlur(r, layer);
    ...
    */
    ...
    function aeArtboard(layer) {
        ...
        if (prefs.newComp) {
            ...
            // create new comp
            thisComp = app.project.items.addComp(nameInc(layer.name, app.project.items), Math.max(Math.round(layer.size[0] * compMult), 4), Math.max(Math.round(layer.size[1] * compMult), 4), 1, 60, frameRate); // pixelAspect=1, duration=60sec, frameRate=60fps
    ...
    function setMask(currentLayer, layerData) {
        ...
        if (maskLayer[layerID]) {
            try {
                ...
                catch (e) { } // if not a shape layer
                newMask.locked = true;
                if (hostApp == 'Figma') {
                    maskLayer[layerID].remove();
                } // figma cant have the underlying mask layer visible so delete it
